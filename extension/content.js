...
var SENT_TIME_PAIR_ELEM = '<div style="color: var(--yt-spec-text-primary); width: 100%; font-size: medium; margin: 15px auto;"><a href="{0}" style="color: #de5f5f; display: inline-block; vertical-align: top; width: 10%;">{1}</a><div style="display: inline-block; width: 80%;">{2}</div></div>'
...
chrome.extension.onMessage.addListener(function(msg, sender, sendResponse) {
    ...
    if (msg.action == "startLoadingWindow") {
      ...
      var base_url = 'https://www.youtube.com/watch?v=' + msg.vidId + '&t='
      for (const [sentence, timestamp] of Object.entries(json)) {
        ...
        var time = timestamp.hours + ":" + timestamp.minutes + ":" + Math.round(timestamp.seconds)
        box.innerHTML += SENT_TIME_PAIR_ELEM.replace('{1}', time).replace('{0}', base_url + seconds + "s").replace('{2}', sentence);
      }
    }
});
