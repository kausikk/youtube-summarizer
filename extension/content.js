...
var SENT_TIME_PAIR_ELEM = '<div style="color: var(--yt-spec-text-primary); width: 100%; font-size: medium; margin: 15px auto; display: flex; justify-content: space-between"><a href="{0}" style="color: #de5f5f; vertical-align: top; width: 8%;">{1}</a><div style="width: 90%; margin-right: 5px;">{2}</div></div>'
...
chrome.extension.onMessage.addListener(function(msg, sender, sendResponse) {
    ...
    if (msg.action == "startLoadingWindow") {
      ...
      for (const [sentence, timestamp] of Object.entries(json)) {
        ...
        box.innerHTML += SENT_TIME_PAIR_ELEM.replace('{1}', time).replace('{0}', '').replace('{2}', sentence);
      }
    }
});
